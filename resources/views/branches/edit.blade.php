@extends('layouts.vertical', ['title' => 'Edit Cabang'])

@section('styles')
    <style>
        .map-placeholder { width:100%; height:180px; border:2px dashed #dee2e6; border-radius:.375rem; display:flex; align-items:center; justify-content:center; background:#f8f9fa; color:#6c757d; }
    </style>
@endsection
...
@section('content')
    ...
    <form action="{{ route('branches.update', $branch->id) }}" method="POST" id="branchForm">
        @csrf
        @method('PUT')
            ...
            <div class="col-lg-4">
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="manager_admin_user_id" class="form-select @error('manager_admin_user_id') is-invalid @enderror">
                                ...
                                @foreach($admins ?? [] as $admin)
                                    <option value="{{ $admin->id }}" {{ (old('manager_admin_user_id', $branch->manager_admin_user_id) == $admin->id) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3 form-check">
                            ...
                            <input type="checkbox" name="is_active" value="1" class="form-check-input" id="isActive" {{ old('is_active', $branch->is_active) ? 'checked' : '' }}>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body small text-muted">
                        <ul>
                            <li>Pastikan kode tetap unik saat mengubah.</li>
                            <li>Isi manager bila admin cabang sudah dibuat.</li>
                        </ul>
                    </div>
                </div>
