...
@section('styles')
    <style>
        ...
        .variant-table td img { border-radius:6px; }

        /* card visuals for variant rows, gallery items */
        .gallery-item.card { border: 1px solid #eef2f7; box-shadow: none; }
        .stacked-cards > .card { margin-bottom: 1rem; }
                ...
                <div class="card-body">
                    <div class="product-gallery" id="productGallery" data-product-id="{{ $product->id }}">
                        @forelse($product->images->sortBy('position') as $img)
                            ...
                            <div class="gallery-item card p-2" data-id="{{ $img->id }}" style="width:160px; position:relative;">
        ...
        <div class="col-12 stacked-cards">
            ...
            <div class="card">
                <div class="card-body d-flex gap-2">
                    @can('products.update')
                        <a href="{{ route('products.edit', $product->id) }}" class="btn btn-block btn-outline-primary w-100">Edit Produk</a>
                    @endcan
