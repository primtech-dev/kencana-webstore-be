@extends('layouts.vertical', ['title' => 'Manajemen Permission'])

@section('styles')
    @vite(['node_modules/datatables.net-responsive-bs5/css/responsive.bootstrap5.min.css'])
@endsection

@section('content')
    @include('layouts.shared.page-title', [
        'title' => 'Manajemen Permission',
        'subTitle' => 'Kelola permission (hak akses)',
        'breadcrumbs' => [
            ['name' => 'Pengaturan', 'url' => '#'],
            ['name' => 'Permission']
        ]
    ])

    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0">Daftar Permission</h5>
                    @can('permissions.create')
                        <a href="{{ route('permissions.create') }}" class="btn btn-primary">
                            <i data-lucide="plus" class="me-1"></i> Tambah Permission
                        </a>
                    @endcan
                </div>

                <div class="card-body">
                    <table class="table table-striped dt-responsive align-middle w-100" id="permissions-table">
                        <thead class="thead-sm text-uppercase fs-xxs">
                        <tr>
                            <th width="5%">No</th>
                            <th>Nama</th>
                            <th>Display Name</th>
                            <th>Group</th>
                            <th width="12%">Dibuat</th>
                            <th width="12%" class="text-center">Aksi</th>
                        </tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>

            </div>
        </div>
    </div>

    <x-delete-modal
        id="deletePermissionModal"
        formId="deletePermissionForm"
        :route="route('permissions.destroy', ':id')"
        itemNameId="delete_permission_title"
        title="Konfirmasi Hapus Permission"
        message="Apakah Anda yakin ingin menghapus permission ini?"
        itemType="permission"
    />
@endsection

@section('scripts')
    @vite(['resources/js/pages/settings/permissions.js'])

    <script>
        window.permissionRoutes = {
            index: '{{ route('permissions.index') }}',
            edit: '{{ route('permissions.edit', ':id') }}',
            destroy: '{{ route('permissions.destroy', ':id') }}'
        };
    </script>
@endsection
